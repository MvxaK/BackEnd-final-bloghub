...
def comment(post_id):
    ...
    sentiment = analyze_sentiment_multilingual(content)

    comment = Comment(content=content, post_id=post_id, user_id=user.id, sentiment=sentiment)
    db.session.add(comment)
    db.session.commit()
    flash(f'Comment submitted. Detected sentiment: {sentiment}', 'info')
...
def analyze_sentiment_multilingual(text):
    ...
    if not isinstance(data, list) or len(data) == 0 or not isinstance(data[0], dict) or 'score' not in data[0]:
